...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(x => x).mapValues(_.size).toList.sorted
   ...
   */
  def until[T](cond: T => Boolean, f: T => T)(ts: T): T =
    if (cond(ts)) ts
    else until(cond, f)(f(ts))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
     ...
     * R = 1
     */
    def decodeChar(t: CodeTree, bs: List[Bit]): (Char, List[Bit]) = t match {
      case Leaf(c, _) => (c, bs)
      case Fork(l, r, _, _) => bs.head match {
        case 0 => decodeChar(l, bs.tail)
        case 1 => decodeChar(r, bs.tail)
        case x => ???
      }
    }
    /**
     * match against result, make sure we don't have bits left
     */
    decodeChar(tree, bits) match {
      case (char, Nil) => List(char)
      case (char, bs) => char :: decode(tree, bs)
    }
  }
  ...
  /**
   * does the tree contain a given character?  this method is inefficient
   * since we will need to add a new case for every new child of CodeTree
   */
  def contains(tree: CodeTree, x: Char): Boolean = tree match {
    case Leaf(c,_) => c == x
    case Fork(_,_,cs,_) => cs.contains(x)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(t: CodeTree, c: Char): List[Bit] = t match {
      case Leaf(_, w) => Nil
      case Fork(l,r,cs,_) => {
        if (contains(l,c)) 0 :: encodeChar(l,c) 
        else 1 :: encodeChar(r,c)
      }
    }
    text.map(c => encodeChar(tree,c)).flatten
  }
